...
const w2 = (sketch) => {
  ...
  function generateGrid(numCols, numRows) { // Done with the help of ChatGPT
    ...
    function generateRoom() {
      const minRoomSize = 3;
      const maxRoomSize = 6;
      const roomWidth = sketch.floor(sketch.random() * (maxRoomSize - minRoomSize + 1)) + minRoomSize;
      const roomHeight = sketch.floor(sketch.random() * (maxRoomSize - minRoomSize + 1)) + minRoomSize;
      const startX = sketch.floor(sketch.random() * (numCols - roomWidth));
      const startY = sketch.floor(sketch.random() * (numRows - roomHeight));
      for (let y = startY; y < startY + roomHeight; y++) {
        for (let x = startX; x < startX + roomWidth; x++) {
          if (sketch.random() < 0.04) {
            grid[y][x] = "c";
          } else {
            grid[y][x] = ".";
          }
    ...
    function connectRooms(room1, room2) {
      const center1X = sketch.floor(room1.startX + room1.roomWidth / 2);
      const center1Y = sketch.floor(room1.startY + room1.roomHeight / 2);
      const center2X = sketch.floor(room2.startX + room2.roomWidth / 2);
      const center2Y = sketch.floor(room2.startY + room2.roomHeight / 2);
      for (let x = sketch.min(center1X, center2X); x <= sketch.max(center1X, center2X); x++) {
        grid[center1Y][x] = ".";
      }
      for (let y = sketch.min(center1Y, center2Y); y <= sketch.max(center1Y, center2Y); y++) {
        grid[y][center2X] = ".";
      }
    }
    for (let i = 0; i < numRows; i++) {
    ...
    const numRooms = sketch.floor(sketch.random() * 5) + 3; // Random number between 3 and 7 rooms
    const rooms = [];
